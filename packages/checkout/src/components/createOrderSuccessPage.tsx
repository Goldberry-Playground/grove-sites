...
/**
 * Build the server-rendered order confirmation page.
 *
 * Each tenant wires this with its own OdooClient. The token still arrives
 * in `searchParams` (URL) — this is a known security smell flagged for a
 * follow-up sprint; the fix is to move the token to a header or POST body
 * and is intentionally NOT done as part of the DRY extraction.
 */
export function createOrderSuccessPage({ odoo }: { odoo: OdooClient }) {
  return async function OrderSuccessPage({
    params,
    searchParams,
  }: {
