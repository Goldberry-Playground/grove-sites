 ...
 *
 * Naming note: the hub tenant is `"gather"` here — matching the caption parser
 * and the manifest's CDN path segment (`gatherLogoSvg` → `/brand/gather/...`).
 * The manifest's `BrandId` getter type in `../index.ts` still calls it `"hub"`;
 * unifying the two brand-id spellings is a follow-up (it touches app consumers
 * and the design-system package) and deliberately out of scope for this seam.
 */

/** Brand namespaces (ingest vocabulary). Mirrors the caption parser; hub = "gather". */
