...
export const marketplace: Marketplace = {
  ...
  featured: [
    ...
    //
    // Retarget, don't seed fake products: feature real, seeded At-The-Grove-
    // Nursery products so the row proves the render path with live cards. Two
    // distinct slots (GOL-440, 2026-07-18): the nursery sticker + the chestnut,
    // the farm's anchor crop. These are different products, not a duplicated
    // card — a lone $2 sticker is a thin sole hero, so the flagship rides
    // alongside it. Swap back to Goldberry slugs once GOL-104 seeds them.
    //
    ...
    // The guard fails the build if any slug below returns count=0.
    {
