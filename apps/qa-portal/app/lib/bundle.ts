...
// Assumes process.cwd() is the apps/qa-portal root (guaranteed by Next under
// next dev/build/standalone; tests bypass this via the injectable baseDir
// param and never rely on this path).
...
export function listComponents(brand: Brand, baseDir: string = PUBLIC_BUNDLES): string[] {
  // "general" is the single group used in slice 1 (all 11 components live
  // here); parameterize if multi-group bundles land.
  const dir = join(baseDir, brand, "components", "general");
  // Bundle not synced yet (predev hook hasn't run) — an empty gallery is the
  // correct render, not an unhandled ENOENT 500.
